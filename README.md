...
Cmd+Left / Cmd+Right snap the front window to the left / right half of its screen, Cmd+Up fills the screen, Cmd+Shift+Up / Cmd+Shift+Down enter / leave real full screen. First use prompts for Accessibility access (System Settings → Privacy & Security → Accessibility). The build signs with a self-signed "Launcher Dev" certificate so that grant survives rebuilds; create one in Keychain Access (Certificate Assistant → Create a Certificate, type Code Signing) if you don't have it.
