...
Cmd+Left / Cmd+Right snap the front window to the left / right half of its screen, Cmd+Up fills the screen, Cmd+Shift+Up / Cmd+Shift+Down enter / leave real full screen, Cmd+1 opens Firefox, Cmd+2 Ghostty (pinned in `Workspaces.swift`), Cmd+3 … Cmd+9 switch to the first, second … other workspace, and Cmd+Shift+Left / Cmd+Shift+Right step through them all. A workspace is an app with a window, numbered in launch order, so switching is instant: no Spaces, no slide. Fill each window with Cmd+Up and it feels like full screen.
